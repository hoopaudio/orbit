...
impl ScreenshotService {
    pub async fn capture_primary_monitor() -> Result<ScreenshotInfo> {
        ...
        let filename = format!("orbit_screenshot_{}.png", timestamp);
    ...
    pub async fn capture_and_analyze() -> Result<ScreenshotAnalysis> {
        let screenshot = Self::capture_primary_monitor().await?;
...
pub async fn take_screenshot() -> Result<ScreenshotInfo, String> {
    ScreenshotService::capture_primary_monitor()
        .await
