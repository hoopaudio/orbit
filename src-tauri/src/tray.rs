...
use crate::{
    commands, toggle_pin, Pinned, TrayMenu, MAIN_WINDOW_NAME, ORBIT, SETTINGS_WINDOW_NAME,
...
impl Tray {
    pub fn create_tray_menu(app_handle: &AppHandle) -> Result<Menu<Wry>, tauri::Error> {
        ...
        MenuBuilder::new(app_handle)
            ...
            .separator()
            .text(ORBIT, format!("Orbit v{version}"))
            .text(TRAY_QUIT, "Quit")
            .build()
    }
    ...
    pub fn update_tray(app_handle: &AppHandle) -> Result<()> {
        ...
        let _ = TrayIconBuilder::with_id(ORBIT)
            .menu(&menu)
